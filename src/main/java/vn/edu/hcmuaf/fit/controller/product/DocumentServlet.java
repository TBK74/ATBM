...
@WebServlet(name = "DocumentServlet", value = "/documents")
public class DocumentServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        int total = DocumentService.getInstance().countDocuments(categoryId, fileType, price, search);
        int totalPages = (int) Math.ceil((double) total / PAGE_SIZE);

        req.setAttribute("documents", DocumentService.getInstance().getDocuments(categoryId, fileType, price, sort, search, page, PAGE_SIZE));
        req.setAttribute("categories", CategoryService.getInstance().getAll());
        req.setAttribute("currentCategory", currentCategory);
        req.setAttribute("selectedFileType", fileType);
